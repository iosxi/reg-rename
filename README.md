...
### 種別ごとの名前のしくみは、アプリ内で読める

「なぜオーディオだけ特別な手順が要るのか」がアプリを使っているだけで
分かるように、メイン画面の**「名前のしくみ...」**ボタン (右クリック
メニューにもある) で種別ごとの違いを 1 画面にまとめてある。

| 種別 | 変更できるか | 特別な手順 |
|---|---|---|
| PnP デバイス名 | 直接書ける | 不要 |
| ネットワーク接続名 | 直接書ける | 不要 (名前の重複だけ注意) |
| **オーディオ endpoint 名** | **前半のみ書ける** | **必要** |

オーディオの名前を変更する画面にも、組み立て名なのはオーディオだけである
旨と、この画面への案内を出している。
